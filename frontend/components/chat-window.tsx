...
import { Send, ShieldOff, UserX, VenetianMask, X } from "lucide-react";
import { useEffect, useRef, useState } from "react";
import { ApiError, chatApi, getCurrentUser, usersApi, type ChatMessage } from "@/lib/api";
  ...
  return (
    ...
    >
      ...
      {isBlocked ? (
        ...
        <form
          onSubmit={handleSend}
          className="flex shrink-0 items-center gap-2 border-t border-slate-200 p-2.5 dark:border-slate-800"
        >
          <input
            type="text"
            value={body}
            ...
            className="flex-1 rounded-full border border-slate-200 bg-slate-100 px-3.5 py-2 text-sm text-slate-700 outline-none focus:border-cyan-400 focus:ring-4 focus:ring-cyan-500/10 disabled:opacity-60 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200"
          />
          <button
            type="submit"
            disabled={!threadId || isSending || !body.trim()}
